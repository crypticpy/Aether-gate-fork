...
def noise_kinds(st, coh, mode, nb_on, blanked_pct, nullable_coherence, filt_status,
    ...
    profile: the NoiseProfile `st` came from, for `since` -- each row's
    `kind_since(key, now)`, the epoch this finding was first seen, moving on
    again only when the profile itself is replaced. None (a caller with
    nothing to persist against, e.g. a fixed test double) leaves every row's
    `since` None rather than a fresh-every-poll epoch that would lie about
    how long the finding has stood.
    Returns the `kinds` list -- st["kinds"] in the caller."""
    directional = coh is not None and coh >= nullable_coherence
    kinds, now = [], time.time()
    kind_since = getattr(profile, "kind_since", None)   # absent on a test double

    def _since(key):
        return kind_since(key, now) if kind_since is not None else None
    ...
    if st["mains_hz"]:
        ...
        kinds.append({
            ...
            "action": act, "why": why, "active": active, "since": _since("mains"),
        })
    if st["impulse_db"] is not None and st["impulses_per_s"] >= 1.0:
        ...
        kinds.append({
            ...
            "action": act, "why": None, "active": bool(nb_on), "since": _since("impulse"),
        })
    for line in st["periodic"]:
        kinds.append({
            ...
            "active": False, "since": _since(f"periodic:{round(line['hz'])}"),
        })
    ...
    if not kinds:
        ...
        kinds.append({
            ...
            "action": act, "why": why, "active": active, "since": _since("floor"),
        })
    return kinds
